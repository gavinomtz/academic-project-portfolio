...
// Program Description: A C++ program that calculates the volume of a cone
...
double volume_cone(int radiusCone, int heightCone);
double round_off(double value, int digits);
void print_data(string name, int radiusCone, int heightCone, double volumeCone);
...
double volume_cone(int radiusCone, int heightCone){
  double volumeCone;
  volumeCone = 1./3. * PI * square(radiusCone) * heightCone;
  return round_off(volumeCone, 3);
}
...
void print_data(string name, int radiusCone, int heightCone, double volumeCone){
  ...
  cout << "\nOk " << name << "\n\nFor a radius: " << radiusCone << " and a height: " << heightCone << " the cone's volume is " << volumeCone << "\n\n";
}